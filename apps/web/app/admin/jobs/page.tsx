...
export default function AdminOpportunitiesPage() {
  ...
  // Modal State & Full Form Fields State
  ...
  const [newExperienceLevel, setNewExperienceLevel] = useState("Mid-Level");
  ...
  // Fetch reference dropdown options (Organizations, Sectors, Categories)
  const fetchMetadataOptions = useCallback(async () => {
    const token = localStorage.getItem("accessToken");
    const headers = token ? { Authorization: `Bearer ${token}` } : {};

    try {
      const [orgRes, secRes, catRes] = await Promise.all([
        fetch("/api/v1/organizations", { headers }).catch(() => null),
        fetch("/api/v1/sectors", { headers }).catch(() => null),
      ...
      if (orgRes && orgRes.ok) {
        ...
        setOrganizations(
          Array.isArray(orgData)
            ? orgData
            : orgData.data || orgData.organizations || [],
        );
      }
      if (secRes && secRes.ok) {
  ...
  }, [fetchOpportunities, fetchMetadataOptions]);
    ...
    try {
      const res = await fetch("/api/v1/jobs", {
        ...
        body: JSON.stringify({
          ...
          description: newDescription || "<p>No description provided.</p>",
  ...
  return (
    <div className="min-h-screen bg-[var(--td-bg-soft)] text-[var(--td-text)]">
      <div className="max-w-6xl mx-auto space-y-8 py-10 px-4">
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs px-4 py-8 overflow-y-auto">
            <div className="max-w-xl w-full bg-[var(--td-bg-surface-elevated)] border border-[var(--td-border-subtle)] rounded-2xl p-6 shadow-2xl space-y-6 my-auto">
              ...
              >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3.5 py-2.5 text-xs bg-[var(--td-bg-soft)] border border-[var(--td-border-subtle)] rounded-xl text-[var(--td-text)] focus:outline-none focus:border-[var(--td-color-primary)] transition-all resize-none"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 text-xs bg-[var(--td-bg-soft)] border border-[var(--td-border-subtle)] rounded-xl text-[var(--td-text)] focus:outline-none focus:border-[var(--td-color-primary)] cursor-pointer"
                    >
